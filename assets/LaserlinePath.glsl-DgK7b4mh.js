import{cp as o,cj as pe,ce as I,kQ as ue,ta as fe,kO as ge,jE as _e,gw as Q,f$ as K,I9 as me,I6 as Pe,ID as ve,Ga as f,Hm as w,Ib as be,Gb as z,G8 as r,eO as U,wh as Z,I8 as we,H4 as S,g$ as $,hq as N,IE as De,mX as Y,Hk as ee,GJ as C,t7 as Ve,gx as m,gy as P,IF as te,t8 as xe,lc as ie,hp as Ee,ga as ye,hE as Le,g_ as Se,cf as ne,fk as se,hn as Ce,t4 as Me,Gc as ae,Gd as re,_ as le,If as Ae,G0 as $e,dD as H,hF as O,I7 as Re,ww as Ie,GH as Te,Hu as Oe,G7 as W,dM as Ue,IG as qe,n as u,Gr as V,Ig as je,Ih as F,IH as ze,lJ as Ne,II as R,IJ as He,hz as We,uB as Fe,hB as Ge,hC as Be,hD as ke,hG as Je,p as M,u as Xe,IK as Qe,dF as G}from"./index-BIS0uLzm.js";import{t as Ke}from"./VisualElement-7HZqCGDy.js";let B=class{constructor(e){this._renderCoordsHelper=e,this._origin=o(),this._dirty=!1,this._count=0}set vertices(e){const n=pe(3*e.length);let i=0;for(const s of e)n[i++]=s[0],n[i++]=s[1],n[i++]=s[2];this.buffers=[n]}set buffers(e){if(this._buffers=e,this._buffers.length>0){const n=this._buffers[0],i=3*Math.floor(n.length/3/2);I(this._origin,n[i],n[i+1],n[i+2])}else I(this._origin,0,0,0);this._dirty=!0}get origin(){return this._origin}draw(e){const n=this._ensureVAO(e);n!=null&&(e.bindVAO(n),e.drawArrays(ue.TRIANGLES,0,this._count))}dispose(){this._vao!=null&&this._vao.dispose()}get _layout(){return this._renderCoordsHelper.viewingMode===1?Ye:et}_ensureVAO(e){return this._buffers==null?null:(this._vao??=this._createVAO(e,this._buffers),this._ensureVertexData(this._vao,this._buffers),this._vao)}_createVAO(e,n){const i=this._createDataBuffer(n);return this._dirty=!1,new fe(e,new ge(e,_e(this._layout),i))}_ensureVertexData(e,n){if(!this._dirty)return;const i=this._createDataBuffer(n);e.buffer()?.setData(i),this._dirty=!1}_createDataBuffer(e){const n=e.reduce((g,c)=>g+k(c),0);this._count=n;const i=this._layout.createBuffer(n),s=this._origin;let a=0,l=0;const d="startUp"in i?this._setUpVectors.bind(this,i):void 0;for(const g of e){for(let c=0;c<g.length;c+=3){const L=I(J,g[c],g[c+1],g[c+2]);c===0?l=this._renderCoordsHelper.getAltitude(L):this._renderCoordsHelper.setAltitude(L,l);const _=a+2*c;d?.(c,_,g,L);const j=Q(J,L,s);if(c<g.length-3){for(let v=0;v<6;v++)i.start.setVec(_+v,j);i.extrude.setValues(_,0,-1),i.extrude.setValues(_+1,1,-1),i.extrude.setValues(_+2,1,1),i.extrude.setValues(_+3,0,-1),i.extrude.setValues(_+4,1,1),i.extrude.setValues(_+5,0,1)}if(c>0)for(let v=-6;v<0;v++)i.end.setVec(_+v,j)}a+=k(g)}return i.buffer}_setUpVectors(e,n,i,s,a){const l=this._renderCoordsHelper.worldUpAtPosition(a,Ze);if(n<s.length-3)for(let d=0;d<6;d++)e.startUp.setVec(i+d,l);if(n>0)for(let d=-6;d<0;d++)e.endUp.setVec(i+d,l)}};function k(t){return 3*(2*(t.length/3-1))}const Ze=o(),J=o(),Ye=K().vec3f("start").vec3f("end").vec2f("extrude").vec3f("startUp").vec3f("endUp"),et=K().vec3f("start").vec3f("end").vec2f("extrude");function oe(t,e){const n=t.fragment;n.include(me),t.include(Pe),n.include(ve),n.uniforms.add(new f("globalAlpha",i=>i.globalAlpha),new w("glowColor",i=>i.glowColor),new f("glowWidth",(i,s)=>i.glowWidth*s.camera.pixelRatio),new f("glowFalloff",i=>i.glowFalloff),new w("innerColor",i=>i.innerColor),new f("innerWidth",(i,s)=>i.innerWidth*s.camera.pixelRatio),new be("depthMap",i=>i.depth?.attachment),new z("normalMap",i=>i.normals)),n.code.add(r`vec4 premultipliedColor(vec3 rgb, float alpha) {
...
return abs(dist);
}`),t.spherical){const i=(a,l,d)=>m(a,l.heightManifoldTarget,d.camera.viewMatrix),s=(a,l)=>m(a,[0,0,0],l.camera.viewMatrix);n.uniforms.add(new S("heightManifoldOrigin",(a,l)=>(i(p,a,l),s(D,l),Q(D,D,p),$(h,D),h[3]=N(D),h)),new De("globalOrigin",a=>s(p,a)),new f("cosSphericalAngleThreshold",(a,l)=>1-Math.max(2,Y(l.camera.eye,a.heightManifoldTarget)*l.camera.perRenderPixelRatio)/N(a.heightManifoldTarget))),n.code.add(r`float globeDistancePixels(float posInGlobalOriginLength) {
